...
const MessageBubble: React.FC<Props> = ({ 
  ...
  // This controls the scale and opacity of the reply icon based on drag distance
  const replyOpacity = useTransform(dragX, [0, 60], [0, 1]);
  const replyScale = useTransform(dragX, [0, 60], [0.5, 1.2]);
  const replyIconX = useTransform(dragX, [0, 60], [-20, 0]);
  ...
  const handleDragEnd = (_: any, info: any) => {
    setIsSwiping(false);
    // If the swipe distance is more than 60px, trigger reply
    if (info.offset.x > 60 && onReply && !isUnsent) {
      onReply(message);
          ...
          <div className={`grid ${gridClass} gap-1.5 mt-0.5 mb-0.5 max-w-[260px] sm:max-w-[320px] relative transform-style-3d`}>
            {mediaItems.map((item, idx) => (
              ...
              >
                ...
                <AnimatePresence>
                  {isSending && (
                    <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.1 }} className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-20 transform-style-3d">
                      <div className="relative w-16 h-16">
                        <svg className="w-full h-full -rotate-90">
                          <circle cx="32" cy="32" r="28" fill="transparent" stroke="rgba(255,255,255,0.1)" strokeWidth="3" />
                          <motion.circle cx="32" cy="32" r="28" fill="transparent" stroke="#3B82F6" strokeWidth="3" strokeDasharray="176" animate={{ strokeDashoffset: 176 - (176 * progress) / 100 }} strokeLinecap="round" className="drop-shadow-[0_0_8px_rgba(59,130,246,0.8)]" />
                        </svg>
  ...
  return (
    <motion.div 
      ...
      className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} select-none relative w-full mb-1`}
    >
      {/* SWIPE REPLY INDICATOR - Positioned absolutely behind the bubble */}
      {!isUnsent && (
        <motion.div 
          style={{ opacity: replyOpacity, scale: replyScale, x: replyIconX }}
          className="absolute left-0 top-0 bottom-0 flex items-center justify-center w-12 pl-2"
        >
      ...
      <div className={`flex gap-3 max-w-[92%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className="relative group flex flex-col">
          ...
          <motion.div 
            drag={!isUnsent ? "x" : false}
            dragConstraints={{ left: 0, right: 100 }} // Limit swipe right
            dragElastic={0.4}
