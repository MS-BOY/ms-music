...
import SwipeableMessage from './SwipeableMessage'; // ← import করো
...
const MessageBubble: React.FC<Props> = ({ message, isMe, showAvatar, onOpenMenu, onMediaClick, onSelectTrack, onReply }) => {
  ...
  const swipeDisabled = isSending || isUnsent || !onReply;
  ...
  return (
    ...
    >
      <div className={`flex gap-3 max-w-[92%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        {/* Swipeable অংশ */}
        <SwipeableMessage onReply={onReply!} isMe={isMe} disabled={swipeDisabled}>
          {showAvatar && !isMe && (
