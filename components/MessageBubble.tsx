...
interface Props {
  ...
  onSwipeReply?: (message: Message) => void; // <-- new prop
}
...
const MessageBubble: React.FC<Props> = ({
  ...
  const renderContent = () => {
    if (isUnsent) {
      return (
        <p className="text-[13px] leading-relaxed text-white/40 italic font-medium">{message.content}</p>
      );
    }
    ...
    switch (message.type) {
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[8px] font-black uppercase tracking-[0.2em] text-cyan-400/80">
                    Shared Music
                  </span>
                </div>
                <h4 className="font-black text-sm text-white uppercase truncate tracking-tight mb-0.5">
                  {trackData.title}
                </h4>
      ...
      case 'image-grid':
        ...
        return (
          <div
            className={`grid ${gridClass} gap-1.5 mt-0.5 mb-0.5 max-w-[260px] sm:max-w-[320px] relative transform-style-3d`}
          >
              ...
              >
                <div
                  className={`w-full h-full transition-all duration-[1s] ${
                    isSending ? 'blur-2xl saturate-150 brightness-50' : 'blur-0'
                  }`}
                >
                  {item.type === 'video' ? (
                ...
                <AnimatePresence>
                  {isSending && (
                    ...
                    >
                      ...
                      <span className="mt-3 text-[9px] font-black uppercase tracking-[0.3em] text-blue-400 drop-shadow-glow">
                        Uploading
                      </span>
                    </motion.div>
                  )}
  ...
  return (
    <motion.div
      ...
      className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} select-none relative`}
    >
      ...
      <div className={`flex gap-3 max-w-[92%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className="relative group flex flex-col">
          ...
          {message.replyTo && !isUnsent && (
            <div
              className={`mb-1.5 px-3 py-2 bg-white/[0.03] rounded-2xl border-l-2 border-blue-500/50 flex flex-col text-[11px] ${
                isMe ? 'self-end mr-1' : 'ml-1'
              }`}
            >
          ...
          <motion.div
            className={`relative overflow-hidden transition-all duration-300 ${
              isMediaOnly ? 'bg-transparent p-0 shadow-none' : 'glass px-4.5 py-3.5 shadow-[0_10px_40px_rgba(0,0,0,0.4)]'
            } ${!isMediaOnly && !isUnsent
              ? isMe
                ? 'bg-blue-600/10 rounded-[28px] rounded-tr-[4px] border-blue-500/10'
                : 'bg-white/[0.05] rounded-[28px] rounded-tl-[4px] border-white/5'
              : ''
            } ${isUnsent ? 'bg-white/[0.02] border border-white/[0.05] p-3 rounded-[24px] italic text-white/40' : ''}`}
          >
            {renderContent()}
            ...
            <div className={`flex items-center gap-2 ${isMediaOnly ? 'mt-1.5 px-2' : 'mt-2'} ${isMe ? 'justify-end' : 'justify-start'}`}>
              {message.isEdited && !isUnsent && (
                <span className="text-[9px] text-white/20 font-black uppercase tracking-widest">Edited</span>
              )}
