      ...
      <AnimatePresence>
        {showEmojiPicker && (
          <motion.div
            ...
            className="absolute bottom-full mb-3 right-3 glass-high p-3 rounded-2xl border border-white/10 shadow-xl bg-[#0a0a0a]/95 backdrop-blur-xl z-50"
          >
      ...
      <AnimatePresence>
        {(replyingTo || editingMessage) && (
          <motion.div
            ...
            className="mb-1"
          >
            <div className={`glass flex flex-col p-2 rounded-xl border ${editingMessage?'border-blue-500/30 bg-blue-500/5':'border-white/10 bg-white/5'}`}>
              ...
              <div className="flex flex-col gap-1">
                {(editingMessage?.content || replyingTo?.content) && (
                  <span className="truncate max-w-[250px] text-white/60">
                    {editingMessage?.content || replyingTo?.content}
                  </span>
                )}
                {(editingMessage?.media || replyingTo?.media)?.map((m,i)=>(
                  <div key={i} className="w-20 h-20 rounded-xl overflow-hidden border border-white/10">
                    {m.type==='video' ? (
                      <video src={m.url} className="w-full h-full object-cover brightness-75" muted playsInline/>
                    ) : (
