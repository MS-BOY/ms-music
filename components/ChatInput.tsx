...
const ChatInput: React.FC<Props> = ({ onSend, onSendMedia, onSendTrack, libraryTracks=[], replyingTo, onCancelReply, editingMessage, onCancelEdit }) => {
  ...
  const removeMedia=(idx:number)=>{
    const item = mediaPreviews[idx]; if(item) URL.revokeObjectURL(item.url);
    ...
    <div className="px-3 py-3 relative z-20 w-full max-w-4xl">
            ...
            <div className={`glass flex items-center justify-between p-2 rounded-xl border ${editingMessage?'border-blue-500/30 bg-blue-500/5':'border-white/10 bg-white/5'}`}>
              <div className="flex items-center gap-2 overflow-hidden">
                ...
                <div className="flex flex-col min-w-0 text-xs">
                  <span className="font-black uppercase tracking-tight flex items-center gap-1">{editingMessage?<Edit2 size={10}/> : <CornerUpLeft size={10}/>} {editingMessage?'Editing':'Replying'}</span>
                  <span className="truncate max-w-[150px] text-white/60">{editingMessage?.content||replyingTo?.content}</span>
                </div>
              </div>
              <button onClick={editingMessage?onCancelEdit:onCancelReply} className="p-1 hover:bg-white/10 rounded-full"><X size={14} className="text-white/40"/></button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          <motion.div initial={{opacity:0,y:10,height:0}} animate={{opacity:1,y:0,height:'auto'}} exit={{opacity:0,y:10,height:0}} className="mb-2 flex overflow-x-auto gap-2">
            {mediaPreviews.map((item,idx)=>(
              <motion.div key={item.url} initial={{scale:0.8,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0,opacity:0}} className="relative w-20 h-20 rounded-xl overflow-hidden border border-white/10">
                ...
                <button onClick={()=>removeMedia(idx)} className="absolute top-1 right-1 w-5 h-5 bg-black/60 rounded-full flex items-center justify-center hover:bg-red-500 border border-white/10"><X size={12} className="text-white"/></button>
              </motion.div>
            ))}
          </motion.div>
        )}
      ...
      <motion.div layout className="glass rounded-2xl border border-white/10 p-1 flex items-end gap-1 bg-[#0a0a0a]/70 backdrop-blur-xl">
        <div className="relative">
          <motion.button whileTap={{scale:0.9,rotate:45}} onClick={()=>setIsExpanded(!isExpanded)} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isExpanded?'bg-white text-black':'bg-white/5 text-white/50 hover:bg-white/10'}`}><Plus size={20}/></motion.button>
          <AnimatePresence>
            {isExpanded && (
        ...
        <textarea ref={inputRef} rows={1} value={text} onChange={handleInputChange} placeholder={replyingTo?"Reply...":editingMessage?"Edit...":"Type a message..."} className="flex-1 bg-transparent border-none outline-none text-sm text-white placeholder:text-white/30 resize-none max-h-24 py-2" onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();handleSend();}}} onFocus={()=>setShowEmojiPicker(false)}/>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <motion.button whileTap={{scale:0.9}} onClick={(text||selectedMedia.length>0)?handleSend:undefined} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${(text||selectedMedia.length>0)?'bg-blue-600 text-white':'bg-white/5 text-white/30 hover:bg-white/10'}`}>{(text||selectedMedia.length>0)?<Send size={16}/>:<Mic size={18}/>}</motion.button>
        </div>
      </motion.div>
